...
Description: Scores each classified news article using FinBERT
             ...
             These four series are the primary explanatory variables
             in the Phase 3 Panel VAR estimation.
...
def score_article(text: str, finbert_pipeline) -> dict:
    ...
    Scoring is applied to the first 2000 characters which captures
    the lead paragraph containing the primary news signal.
    """
    if not text or len(text.strip()) < 20:
        return {"label": "neutral", "score": 0.0, "signed_score": 0.0}

    result = finbert_pipeline(text[:2000])[0]
...
def score_corpus(df: pd.DataFrame,
    ...
    df["sentiment_score"] = scores   # Primary variable: signed in [-1, +1]
...
def aggregate_daily_sentiment(df: pd.DataFrame,
    ...
    Weight = word count (longer articles receive higher weight).

    Output columns: S_Monetary | S_Fiscal | S_External | S_Energy
    One row per trading day. Missing days carry forward the last
    observed value (forward-fill).
    """
    ...
    if not daily_scores:
        print("No classified articles available — corpus required")
        return pd.DataFrame()
    ...
    merged[score_cols] = merged[score_cols].ffill()   # forward-fill missing days
...
def plot_sentiment_series(daily_df: pd.DataFrame,
    ...
    for ax, col, color, label in zip(axes, score_cols, colors, labels):
        ...
        ax.set_title(f"Daily {label} Sentiment", fontsize=11)
...
def save_sentiment_series(daily_df: pd.DataFrame,
                           output_path: str = "daily_sentiment.xlsx"):
    """
    Save the four daily sentiment series to Excel.
    This file is the primary input to Phase 3 (Panel VAR) estimation.
    """
    daily_df.to_excel(output_path, index=False)
    print(f"\n--- Sentiment Series Saved ---")
    print(f"Output    : {Path(output_path).resolve()}")
    print(f"Shape     : {daily_df.shape}")
    print(f"Date range: {daily_df['date_only'].min()} to {daily_df['date_only'].max()}")
...
if __name__ == "__main__":
    ...
    # Input: output of topic_router.py (Phase 1)
    input_path = Path("classified_articles.csv")
    ...
    if input_path.exists():
        ...
        save_sentiment_series(daily_df, output_path="daily_sentiment.xlsx")
    else:
        print("\nCorpus file not found.")
        print("Run topic_router.py (Phase 1) first to generate classified_articles.csv")
