...
Module : Phase 4 — Panel VAR, Granger Causality, IRF, FEVD
    ...
    for sector in SECTORS:
        ...
        for category in CATEGORIES:
            ...
            try:
                ...
                # Collect F-stat p-value at each lag
...
def estimate_var(panel: pd.DataFrame, lag_order: int = None) -> object:
    ...
    if lag_order is None:
        ...
        print(f"Decision: AIC selects lag order = {lag_order}")
    else:
...
def compute_irf(var_result, periods: int = 22,
    ...
    Key interpretation:
      Positive IRF peak before t=0  → informational leakage (pre-announcement)
      Positive IRF peak at t=1–3    → rapid absorption (efficient)
      Positive IRF peak at t=5–22   → slow absorption (illiquid sector)
    """
    print(f"\n--- Computing Impulse Response Functions ({periods} periods) ---")

    irf = var_result.irf(periods=periods)

    # Plot IRFs for each sentiment → sector pair
...
def compute_fevd(var_result, horizons: list = FEVD_HORIZONS) -> pd.DataFrame:
    ...
    fevd_tbl = fevd.decomp  # shape: (variables, periods, variables)
    ...
    for r_col in return_cols:
        ...
        for s_col in sentiment_cols:
            ...
            for h in horizons:
                ...
                row[f"FEVD_h{h}"] = round(share * 100, 2)  # Express as percent
...
if __name__ == "__main__":
    ...
    print("  ../data/processed/panel_data.xlsx  (merged market + sentiment)")

    panel_path = Path("../data/processed/panel_data.xlsx")
    ...
    if panel_path.exists():
        ...
        gc_df.to_excel("../05_results/granger_causality.xlsx", index=False)

        # VAR estimation
        var_result = estimate_var(panel)

        # IRF
        irf = compute_irf(var_result, periods=22,
                          output_path="../05_results/irf_plots.png")

        # FEVD
        fevd_df = compute_fevd(var_result)
        fevd_df.to_excel("../05_results/fevd_results.xlsx", index=False)

        print("\nDecision: All Phase 4 outputs saved → 05_results/")
    else:
        print("\nDecision: Panel data not yet available.")
        print("Phases 1–3 must complete first.")
